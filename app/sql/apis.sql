-- =============================================================================
-- Qlik Cloud Native App - MCP Call Procedure (with User Impersonation)
-- =============================================================================
...
logger = logging.getLogger("qlik_mcp")

def get_tenant(session):
    logger.info("get_tenant: reading qlik_tenant from internal.config")
    row = session.sql("SELECT value FROM internal.config WHERE key = 'qlik_tenant'").collect()
    if not row or not row[0][0]:
        logger.warning("get_tenant: qlik_tenant not configured")
...
def get_user_subject(session, sf_user):
    ...
    # Look up the Qlik subject ID in the mapping table
    row = session.sql(f"SELECT qlik_subject FROM internal.user_subject_map WHERE snowflake_user = '{sf_user}'").collect()
    if row and row[0][0]:
    ...
    raise Exception(f"No Qlik subject mapping for Snowflake user '{sf_user}'. Add a mapping in the app UI.")
...
def get_user_token(tenant, client_id, client_secret, subject):
    token_url = f"https://{tenant}/oauth/token"
    logger.info(f"get_user_token: POST {token_url} grant_type=urn:qlik:oauth:user-impersonation user_lookup.subject={subject}")
    resp = requests.post(token_url, json={
